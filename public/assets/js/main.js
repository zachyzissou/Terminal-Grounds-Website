...
async function evaluateImageQuality(img, category) {
    ...
    let esum = 0; for (const ev of edges) { const d = ev - emean; esum += d * d; }
        ...
        // Category thresholds (tunable) - STRICT MODE
        const cat = (category || '').toLowerCase();
        const req = {
            environments: { contrast: 80, sharp: 2000, color: 15 },
            renders:      { contrast: 80, sharp: 2000, color: 12 },
            vehicles:     { contrast: 80, sharp: 2000, color: 10 },
            weapons:      { contrast: 80, sharp: 2000, color: 10 },
            factions:     { contrast: 80, sharp: 2000, color: 4  }, // emblems
            ui:           { contrast: 80, sharp: 2000, color: 2  },
            characters:   { contrast: 80, sharp: 2000, color: 10 },
            others:       { contrast: 80, sharp: 2000, color: 8  }
        }[cat] || { contrast: 80, sharp: 2000, color: 8 };
